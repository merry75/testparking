...
class AddParkingDialog extends React.Component {
  ...
  handleSubmit(errors, values) {
    ...
    if (values.length === 0) {
      this.props.Add([
        this.state.parkingAreaName,
        this.state.hourlyWeekdayRate,
        this.state.hourlyWeekendRate,
        this.state.discountRate,
        this.state.parkingAreaImage
      ]);
      this.setState({ modal: !this.state.modal });
      this.setState({ parkingAreaName: "" });
      this.setState({ hourlyWeekdayRate: 0 });
    ...
    return (
      <span>
        ...
        <Modal
          ...
          size="md"
        >
          <ModalHeader toggle={this.toggle}>Create Parking Area</ModalHeader>
          <ModalBody>
            <AvForm onSubmit={this.handleSubmit}>
              ...
              <AvGroup row>
                <Col sm={4}>
                  <Label check for="hourlyWeekdayRate">
                    Hourly Weekday Rate
                  </Label>
                </Col>
                <Col sm={8}>
                  <AvField
                    type="number"
                    name="hourlyWeekdayRate"
                    onChange={this.changeWeekdayRate}
                    placeholder="$0"
                    required
                  />
                </Col>
              ...
              <AvGroup row>
                <Col sm={4}>
                  <Label check for="hourlyWeekendRate">
                    Hourly Weekend Rate
                  </Label>
                </Col>
                <Col sm={8}>
                  <AvField
                    type="number"
                    name="hourlyWeekendRate"
                    onChange={this.changeWeekendRate}
                    placeholder="$0"
                    required
                  />
                </Col>
              ...
              <AvGroup row>
                <Col sm={4}>
                  <Label check for="parkingAreaImage">
                    Parking Area Image
                  </Label>
                </Col>
                <Col sm={8}>
...
const mapDispatchToProps = dispatch => {
  return {
    Add: value => {
      dispatch(actions.addParkingArea(value));
    }
  };
};
