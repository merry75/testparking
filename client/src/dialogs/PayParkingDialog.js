...
class PayParkingDialog extends React.Component {
  ...
  toggle() {
    ...
    this.setState(prevState => ({
      errors: { ...prevState.errors, start_date: false }
    }));
    this.setState(prevState => ({
      errors: { ...prevState.errors, start_time: false }
    }));
    this.setState(prevState => ({
      errors: { ...prevState.errors, end_date: false }
    }));
    this.setState(prevState => ({
      errors: { ...prevState.errors, end_time: false }
    }));
  }
  ...
  handleCalculate() {
    ...
    if (this.state.start_date === "")
      this.setState(prevState => ({
        errors: { ...prevState.errors, start_date: true }
      }));
    if (this.state.start_time === "")
      this.setState(prevState => ({
        errors: { ...prevState.errors, start_time: true }
      }));
    if (this.state.end_date === "")
      this.setState(prevState => ({
        errors: { ...prevState.errors, end_date: true }
      }));
    if (this.state.end_time === "")
      this.setState(prevState => ({
        errors: { ...prevState.errors, end_time: true }
      }));
    ...
    if (this.state.end_date !== this.state.start_date) {
      var startDayRestTime = moment(this.state.start_date + " 24:00").diff(
        moment(this.state.start_date + " " + this.state.start_time),
        "hours"
      );
      if (startDateTime.weekday() > 0 && startDateTime.weekday() < 6)
        //case weekday
        pay_amount = startDayRestTime * weekday_rate;
      //case weekend
        ...
        >
          ...
          <ModalBody>
            <AvForm onSubmit={this.handleSubmit}>
              <AvGroup row>
                <Col sm={3}>
                  <Label check for="parkingAreaName">
                    Parking Area Name
                  </Label>
                </Col>
                <Col sm={9}>
                  <AvField
                    ...
                    onChange={this.handleSelectParkingArea}
                    placeholder="Parking area name"
                  >
                    {this.props.defaultData.map(parkingArea => {
                      return (
                        <option value={parkingArea.id} key={parkingArea.id}>
                          {parkingArea.name}
                        </option>
                      );
                    })}
              ...
              <AvGroup row>
                <Col sm={3}>
                  ...
                  this.state.errors.start_time ? (
                    <Label className="text-danger" for="startTime">
                      Start time
                    </Label>
                  ) : (
                ...
                <Col sm={9}>
                  <div className="position-relative form-group">
                    {this.state.errors.start_date ? (
                      <input
                        type="date"
                        className="form-control av-valid is-invalid col-sm-6"
                      ...
                      <input
                        type="date"
                        className="form-control av-valid col-sm-6"
                        onChange={this.handleChangeStartDate}
                        value={this.state.start_date}
                        style={{ float: "left", marginRight: "10px" }}
                        required
                    ...
                    {this.state.errors.start_time ? (
                      <input
                        type="time"
                        className="form-control av-valid is-invalid col-sm-5"
                      ...
                      <input
                        type="time"
                        className="form-control av-valid col-sm-5"
                        onChange={this.handleChangeStartTime}
                        value={this.state.start_time}
                        required
                      />
                    ...
                    {this.state.errors.start_date ||
                    this.state.errors.start_time ? (
                      <div className="invalid-datetime">
              ...
              <AvGroup row>
                <Col sm={3}>
                  {this.state.errors.end_date || this.state.errors.end_time ? (
                    <Label className="text-danger" for="endTime">
                      End time
                    </Label>
                  ) : (
                ...
                <Col sm={9}>
                  {this.state.errors.end_date ? (
                    <input
                      type="date"
                      className="form-control is-invalid col-sm-6"
                  ...
                  {this.state.errors.end_time ? (
                    <input
                      type="time"
                      className="form-control is-invalid col-sm-5"
              ...
              <AvGroup style={{ textAlign: "right" }}>
                {this.state.calculated ? (
                  <Button color="primary" style={{ marginRight: "7px" }}>
                    Book
                  </Button>
                ) : (
                  ""
...
const mapDispatchToProps = dispatch => {
  return {
    Pay: value => {
      dispatch(actions.payParkingArea(value));
    }
  };
};
